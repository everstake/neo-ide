...
class BaseFile extends React.Component<any, any> {
    static propTypes = {
    ...
    componentDidUpdate(oldProps, oldState) {
        if (!oldProps.isRenaming && this.props.isRenaming) {
            window.requestAnimationFrame(() => {
                const currentName = (this as any).newNameRef.value;
                const pointIndex = currentName.lastIndexOf(".");
                (this as any).newNameRef.setSelectionRange(0, pointIndex || currentName.length);
                (this as any).newNameRef.focus();
            });
        }
    }
    ...
    handleNewNameChange = (event) => {
        const newName = (this as any).newNameRef.value;
