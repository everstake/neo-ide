...
class RawFileBrowser extends React.Component<any, any> {
    static propTypes = {
    ...
    handleGlobalClick = (event) => {
        ...
        const inBrowser = !!((this as any).browserRef && (this as any).browserRef.contains(event.target));

        // TODO: updated old-to-new ref styles, but this ref was never set
        const inPreview = !!((this as any).previewRef && (this as any).previewRef.contains(event.target));
    ...
    handleActionBarUploadFileClick = (event) => {
        event.preventDefault();
        this.props.onUploadFile(event);
    }
    ...
    renderActionBar(selectedItem) {
        const {
            ...
            filterRenderer: FilterRenderer, onCreateFolder, onUploadFile, onCreateFiles,
    ...
    render() {
        ...
        let header;
        let contents: any = this.renderFiles(files, 0);
