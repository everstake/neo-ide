...
export const format = function (fmt: string, dateNumber: string) {

  const dateTimer = formatUnixTime(dateNumber.toString());
  const dateObj = new Date(dateTimer);
  // 如果是英文
  if (intl.currentLang === Language.EN) {
    ...
    return day + " " + monthArray[ month ] + ", " + dateObj.getUTCFullYear() + " " + hour + ":" + minute + ":" + second + "";
  }
